...
int32_t libmpq__archive_open(mpq_archive_s **mpq_archive, const char *mpq_filename, libmpq__off_t archive_offset) {
    ...
    rc = (*mpq_archive)->mpq_header.hash_table_count * sizeof(mpq_hash_s);
    rb = fread((*mpq_archive)->mpq_hash, 1, rc, (*mpq_archive)->fp);
    if (rb < rc) {
    ...
    rc = (*mpq_archive)->mpq_header.block_table_count * sizeof(mpq_block_s);
    rb = fread((*mpq_archive)->mpq_block, 1, rc, (*mpq_archive)->fp);
    if (rb < rc) {
    ...
    if ((*mpq_archive)->mpq_header_ex.extended_offset > 0) {
        ...
        /* read header from file. */
        rc = (*mpq_archive)->mpq_header.block_table_count * sizeof(mpq_block_ex_s);
        rb = fread((*mpq_archive)->mpq_block_ex, 1, rc, (*mpq_archive)->fp);
        if (rb < rc) {
