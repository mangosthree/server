...
class Profile:
    ...
    check_codes: tuple[int, int, int, int]
    provenance_note: str
...
PROFILES = {
    ...
    "x64": Profile(
        ...
        mode="compatibility-probe-only",
        mode_enum="CompatibilityProbeOnly",
        assurance="static-verified",
        assurance_enum="StaticVerified",
        ...
        check_codes=(0xEA, 0x00, 0x00, 0x00),
        provenance_note=(
...
def generate_source(profile: Profile, data: bytes, key: bytes) -> str:
    timing, lua, mpq, memory = profile.check_codes
    return f"""{LICENSE}
/*
...
{{
    ...
    profile.checkCodes = {{0x{timing:02X}, 0x{lua:02X}, 0x{mpq:02X}, 0x{memory:02X}}};
    profile.rekey.seed = {{{{
