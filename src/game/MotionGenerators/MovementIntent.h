 ...
 * should this unit head and how should it face -- as a pure MoveIntent value. It
 * never touches the spline, the path finder or a packet. The MotionDriver
...
namespace Motion
{
     ...
     *
     * The client derives facing from the spline it interpolates: it faces the
     * direction of travel while moving, and applies whichever of these forms the
     * SMSG_MONSTER_MOVE carried once the spline ends. "Face the player" is therefore
     * not a separate action -- it is the final facing of a leg.
     */
    struct Facing
    {
    ...
    enum MoveFlags : uint32
    {
        ...
        /// Refuse the leg when the router found no real path, instead of taking its
        /// straight-line fallback. Chase, follow, flee and patrol all want this -- a mob
        /// must not walk through a wall to reach you; a point, wander or home move
    ...
    struct MoveIntent
    {
        ...
        /// Move: exact geometry for the leg, when the generator must dictate it rather
        /// than name a point and let the driver route there. Only the smoothed waypoint
        /// patrol needs it, to weld several nodes into one spline.
        ///
        /// NON-OWNING, and it must stay stable for as long as the leg is walked: the
        /// driver may re-lay from it, and rebuilding it mid-leg from a different start
        /// would walk the unit back to the beginning of its own path.
    ...
    /**
     * @brief The read-only view of leg progress the driver hands the generator, so it
     *        can react to its leg without ever seeing the spline.
     *
