 ...
 *
 * "Go there; when the leg ends, I am done" is all an intent-model one-shot ever is. The
 * variants below change only the flavour of the leg and what happens at the end, which
 * is why they override nothing but LegFlags and Finalize.
 */
...
/**
 * @brief A creature running to fetch help. It walks, so the players it is fetching have
 *        a chance to catch it, and it calls that help when it gets there.
 */
...
/**
 * @brief Guards a spline that something ELSE launched -- a knockback, a jump, a scripted
 *        effect.
 *
 * It has no destination of its own to want, so its intent is the minimal one: hold while
 * that spline is still playing out, and be done the moment it is not. That is what stops
 * the generator underneath from interrupting the effect mid-flight.
 */
