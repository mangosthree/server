...
namespace
{
    /// A node that could not be routed to is skipped after this pause, rather than butted
    /// into forever.
     ...
     * straight-line fallback means we do not know what lies between the nodes, and welding
     * through it would walk the creature into it.
     *
                                                ...
                                                WaypointPath::const_iterator startPoint)
{
    ...
    // budget rather than stopping at a fixed count, so the spline carries as many
    // waypoints as the SMSG_MONSTER_MOVE encoding safely allows. This minimises spline
    // finalize/relaunch boundaries -- each of which is a visible stop and restart -- while
    // guaranteeing the packed offsets never wrap.
    ...
    for (size_t segment = 0; segment < WAYPOINT_SMOOTHING_MAX_LOOKAHEAD; ++segment)
    {
        ...
        {
            // The FIRST leg failing means nothing is usable, so fall back to a plain routed
            // leg. A later one failing keeps the chunk built so far.
            if (committed == 0)
            {
                ClearSegment();
                m_legPoints.clear();
...
Motion::MoveIntent WaypointMovementGenerator::PrepareMove(Creature& creature)
{
    ...
    // A patrol walks unless the DB says it runs. Keep the unit's own walk flag in step with
    // the leg's pace -- the client reads it for the animation, the driver reads MOVE_WALK
    // for the speed.
                                                     ...
                                                     uint32 diff)
{
    ...
    // Mid-leg: re-state the leg we are already walking. The driver recognises the unchanged
    // goal and leaves the running spline alone.
                                                 ...
                                                 float& o) const
{
    ...
    // than the last waypoint it reached. That removes the visible backtrack where an
    // evading patroller runs back to the previous waypoint and then re-walks the leg it was
    // already on. A zero sentinel means no combat start was recorded.
    float combatX, combatY, combatZ;
    const Geometry::Vector3& combatAnchor = creature.CombatAnchor();
    combatX = combatAnchor.x;
    combatY = combatAnchor.y;
    combatZ = combatAnchor.z;
    ...
    // Before the first waypoint is reached m_lastReachedWaypoint is 0, which may not be in
    // the path at all.
...
bool WaypointMovementGenerator::SetNextWaypoint(uint32 pointId)
{
    ...
    // Move on the next tick. Doing it this way does not disturb a PAUSED state: if this is
    // called while paused, the unit moves properly once unpaused.
    m_nextMoveTime.Reset(1);
    m_isArrivalDone = false;
    ClearSegment();
    m_haveLeg = false;
