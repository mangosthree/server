...
// the hull's, and the biggest of these is a gunship at roughly 220 yards stem to stern.
...
class WorldObject : public Object
{
    ...
    public:
        ...
        /// A VEHICLE SEAT, and only ever that. Nothing aboard a ship has one: she is a
        /// map, and what is on a map is simply on it.
        TransportInfo* GetTransportInfo() const { return m_transportInfo; }
        bool IsBoarded() const { return m_transportInfo != NULL; }
        /// Boarding puts the object in its vehicle's frame (VehicleInfo::Board does
        /// that); stepping off must put it back in its map's, or its coordinates go on
        /// being read as seat offsets by everything that asks.
...
// world membership and phasing is game state, line of sight is a terrain question, and a map's
