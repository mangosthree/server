    ...
    switch (m_script->command)
    {
        ...
        case SCRIPT_COMMAND_MOVE_DYNAMIC:                   // 37
        {
            ...
            else                                            // Calculate position
            {
                ...
                const Geometry::Vector3 spot = RandomGroundPointNear(*pSource, pTarget->Where().Pos(), m_script->moveDynamic.maxDist, m_script->moveDynamic.minDist, (orientation == 0.0f ? NULL : &orientation));
                x = spot.x;
                y = spot.y;
                z = spot.z;
