...
void Spell::cast(bool skipCheck)
{
    ...
    switch(m_spellInfo->GetSpellFamilyName())
    {
        ...
        {
            // Ice Block
            if (classOpt && classOpt->SpellClassMask & UI64LIT(0x0000008000000000))
            {
        ...
        {
            // Shield Slam
            if (classOpt && (classOpt->SpellClassMask & UI64LIT(0x0000020000000000)) && m_spellInfo->GetCategory()==1209)
            {
            ...
            // Bloodrage
            if (classOpt && (classOpt->SpellClassMask & UI64LIT(0x0000000000000100)))
            {
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            if (m_spellInfo->GetMechanic() == MECHANIC_SHIELD &&
                (classOpt && classOpt->SpellClassMask & UI64LIT(0x0000000000000001)))
                AddPrecastSpell(6788);                      // Weakened Soul
            // Prayer of Mending (jump animation), we need formal caster instead original for correct animation
            else if (classOpt && classOpt->SpellClassMask & UI64LIT(0x0000002000000000))
            {
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            else if (classOpt && classOpt->SpellClassMask & UI64LIT(0x0000000000400080))
            {
                AddPrecastSpell(25771);                     // Forbearance
                AddPrecastSpell(61987);                     // Avenging Wrath Marker
            }
            // Lay on Hands
            else if (classOpt && classOpt->SpellClassMask & UI64LIT(0x0000000000008000))
            {
                // only for self cast
            ...
            // Avenging Wrath
            else if (classOpt && classOpt->SpellClassMask & UI64LIT(0x0000200000000000))
            {
                AddPrecastSpell(61987);                     // Avenging Wrath Marker
            }
            break;
        ...
        case SPELLFAMILY_SHAMAN:
        {
            ...
            // Totem of Wrath
            else if (spellEffect && spellEffect->Effect==SPELL_EFFECT_APPLY_AREA_AURA_RAID && classOpt && classOpt->SpellClassMask & UI64LIT(0x0000000004000000))
