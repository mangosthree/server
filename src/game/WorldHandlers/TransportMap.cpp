...
namespace
{
    /// The map id an update packet built here must carry: the WORLD map the vessel sails.
    /// This client was never told the deck map exists and drops any packet naming another
    /// map -- so the one place a hull's own id may never appear is the wire.
    uint16 WorldMapIdOf(Transport const* vessel)
    {
        return uint16(vessel && vessel->GetMap() ? vessel->GetMapId() : 0);
    }
    ...
    void DrawMinionTo(Unit* minion, Unit* master, Map* dest)
    {
        ...
        {
            DEBUG_FILTER_LOG(LOG_FILTER_DECK_MINIONS,
                             "DrawMinionTo: SKIP not-drawable %s",
                             DescribeSpatially(minion).c_str());
            return;
        }
        ...
        DEBUG_FILTER_LOG(LOG_FILTER_DECK_MINIONS,
                         ...
                         dest->AsTransport() ? "[deck]" : "",
                         DescribeSpatially(master).c_str());
...
void TransportMap::UpdateMinions()
{
    ...
    for (Creature* minion : stranded)
    {
        ...
        DEBUG_FILTER_LOG(LOG_FILTER_DECK_MINIONS,
                         "UpdateMinions: STRANDED aboard %s",
                         DescribeSpatially(minion).c_str());
...
void TransportMap::AnnounceVessel(Transport* vessel, Player* observer)
{
    ...
    UpdateData data(WorldMapIdOf(vessel));
