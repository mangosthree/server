    ...
    std::unordered_map<uint32, uint32> const& ShippedVesselMaps()
    {
        static const std::unordered_map<uint32, uint32> shipped = []
        {
            ...
            for (uint32 i = 0; i < sMapStore.GetNumRows(); ++i)
            {
                ...
                // Every DBC string here is an array of MAX_LOCALE pointers, filled at
                // whichever locale was loaded and "" everywhere else. The directory is not
                // localised in the file at all, so the first non-empty slot is the name.
                for (uint32 loc = 0; loc < MAX_LOCALE; ++loc)
                {
                    uint32 owner = 0;
                    if (entry->Directory[loc] && entry->Directory[loc][0] &&
                        std::sscanf(entry->Directory[loc], "Transport%u", &owner) == 1 && owner)
                    {
...
void Transport::RegisterVesselMap(uint32 goEntry, char const* vesselName)
{
    ...
    // A locale array per string field, as the loader itself builds -- the row is read by
    // the same code that reads Blizzard's, and that code indexes by locale.
    static std::list<std::vector<char const*>> s_strings;
    s_strings.push_back(std::vector<char const*>(MAX_LOCALE, ""));
    char const** directory = s_strings.back().data();
    s_strings.push_back(std::vector<char const*>(MAX_LOCALE, s_names.back().c_str()));
    char const** mapName = s_strings.back().data();
    ...
    row->MapType = 0;
    row->PVP = 0;
    row->MapName_lang = mapName;
    row->AreaTableID = 0;
    row->MapDescription0_lang = directory;
    row->MapDescription1_lang = directory;
    row->LoadingScreenID = 0;
    row->MinimapIconScale = 0.0f;
    row->CorpseMapID = -1;
    row->Corpse_0 = 0.0f;
    row->Corpse_1 = 0.0f;
    row->TimeOfDayOverride = 0;
    row->ExpansionID = 0;
    row->RaidOffset = 0;
    row->MaxPlayers = 0;
    row->ParentMapID = -1;
...
bool Transport::GenerateWaypoints(uint32 pathid, std::set<uint32>& mapids)
{
    ...
    for (size_t i = 1; i < keyFrames.size(); ++i)
    {
        ...
        else
        {
            keyFrames[i].distFromPrev =
                sqrt(pow(keyFrames[i].node->Loc_0 - keyFrames[i - 1].node->Loc_0, 2) +
                     pow(keyFrames[i].node->Loc_1 - keyFrames[i - 1].node->Loc_1, 2) +
                     pow(keyFrames[i].node->Loc_2 - keyFrames[i - 1].node->Loc_2, 2));
        }
    ...
    WayPoint pos(keyFrames[0].node->ContinentID, keyFrames[0].node->Loc_0, keyFrames[0].node->Loc_1, keyFrames[0].node->Loc_2, teleport,
    ...
    for (size_t i = 0; i < keyFrames.size() - 1; ++i)
    {
        ...
        if (((d < keyFrames[i + 1].distFromPrev) && (tTo > 0)))
        {
            while ((d < keyFrames[i + 1].distFromPrev) && (tTo > 0))
            {
                ...
                if (d > 0)
                {
                    ...
                    newX = keyFrames[i].node->Loc_0 + (keyFrames[i + 1].node->Loc_0 - keyFrames[i].node->Loc_0) * d / keyFrames[i + 1].distFromPrev;
                    newY = keyFrames[i].node->Loc_1 + (keyFrames[i + 1].node->Loc_1 - keyFrames[i].node->Loc_1) * d / keyFrames[i + 1].distFromPrev;
                    newZ = keyFrames[i].node->Loc_2 + (keyFrames[i + 1].node->Loc_2 - keyFrames[i].node->Loc_2) * d / keyFrames[i + 1].distFromPrev;
        ...
        WayPoint pos(keyFrames[i + 1].node->ContinentID, keyFrames[i + 1].node->Loc_0, keyFrames[i + 1].node->Loc_1, keyFrames[i + 1].node->Loc_2, teleport,
