...
SpellAuraProcResult Unit::HandleDummyAuraProc(Unit* pVictim, uint32 damage, Aura* triggeredByAura, SpellEntry const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
    ...
    uint32 rangeIndex = procSpell->rangeIndex;
    uint32 dummySpellIcon = dummySpell->SpellIconID;
    ...
    switch(dummySpell->GetSpellFamilyName())
    {
        ...
        {
            // Magic Absorption
            if (dummySpellIcon == 459)             // only this spell have SpellIconID == 459 and dummy aura
            {
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (dummyClassOptions && dummyClassOptions->SpellFamilyFlags == UI64LIT(0x0) && dummySpellIcon == 1932)
            {
                Modifier* mod = triggeredByAura->GetModifier();
        ...
        case SPELLFAMILY_ROGUE:
        {
            ...
            // Deadly Brew
            if (dummySpellIcon == 2963)
            {
                triggered_spell_id = 44289;
                break;
            }
            // Quick Recovery
            if (dummySpellIcon == 2116)
            {
                if (!procSpell)
                {
                    return SPELL_AURA_PROC_FAILED;
        ...
        case SPELLFAMILY_SHAMAN:
        {
            ...
            // Lightning Overload
            if (dummySpellIcon == 2018)            // only this spell have SpellFamily Shaman SpellIconID == 2018 and dummy aura
            {
...
SpellAuraProcResult Unit::HandleProcTriggerSpellAuraProc(Unit* pVictim, uint32 damage, Aura* triggeredByAura, SpellEntry const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    SpellClassOptionsEntry const* procClassOptions = procSpell ? procSpell->GetSpellClassOptions() : NULL;
    uint32 auraSpellIcon = auraSpellInfo->SpellIconID;
    uint32 auraSpellVisual = auraSpellInfo->SpellVisual[0];
    ...
    switch (auraSpellInfo->GetSpellFamilyName())
    {
        ...
        case SPELLFAMILY_MAGE:
        {
            if (auraSpellIcon == 2127)         // Blazing Speed
            {
                switch (auraSpellInfo->Id)
                {
                    case 31641:  // Rank 1
            ...
            else if (auraSpellIcon == 2947)     // Fingers of Frost
            {
        ...
        case SPELLFAMILY_WARRIOR:
        {
            ...
            if (auraClassOptions && auraClassOptions->SpellFamilyFlags == UI64LIT(0x0) && auraSpellIcon == 243)
            {
                float weaponDamage;
            ...
            else if (auraSpellIcon == 2961)    // Taste for Blood
            {
                // only at real damage
                if (!damage)
                {
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (auraSpellInfo->IsFitToFamilyMask(UI64LIT(0x0000000000004000)))
            {
                ...
                for (Unit::AuraList::const_iterator i = mDummyAura.begin(); i != mDummyAura.end(); ++i)
                {
                    ...
                    if ((*i)->GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_WARLOCK && itrSpellProtoIcon == 113)
                    {
        ...
        case SPELLFAMILY_ROGUE:
        {
            if (auraSpellIcon == 2260)         // Combat Potency
            {
        ...
        {
            // Piercing Shots
            if (auraSpellIcon == 3247 && auraSpellVisual == 0)
            {
    ...
    uint32 powerType = procSpell->powerType;
    ...
    switch (trigger_spell_id)
    {
        ...
        case 35095:
        {
            ...
            if(!spellPower || !procSpell || powerType!=POWER_MANA || spellPower->manaCost==0 && spellPower->ManaCostPercentage==0 && spellPower->manaCostPerlevel==0)
            {
                return SPELL_AURA_PROC_FAILED;
            }
            break;
...
SpellAuraProcResult Unit::HandleProcTriggerDamageAuraProc(Unit* pVictim, uint32 /*damage*/, Aura* triggeredByAura, SpellEntry const* /*procSpell*/, uint32 /*procFlags*/, uint32 /*procEx*/, uint32 /*cooldown*/)
{
    ...
    SpellNonMeleeDamage damageInfo(this, pVictim, spellInfo->Id, SpellSchoolMask(schoolMask));
...
SpellAuraProcResult Unit::HandleOverrideClassScriptAuraProc(Unit* pVictim, uint32 /*damage*/, Aura* triggeredByAura, SpellEntry const* procSpell, uint32 /*procFlag*/, uint32 /*procEx*/ , uint32 cooldown)
{
    ...
    uint32 procSpellVisual = procSpell->SpellVisual[0];

    switch (scriptId)
    {
        case 836:                                           // Improved Blizzard (Rank 1)
        {
            if (!procSpell || procSpellVisual != 9487)
            {
                return SPELL_AURA_PROC_FAILED;
            }
...
SpellAuraProcResult Unit::HandleMendingAuraProc(Unit* /*pVictim*/, uint32 /*damage*/, Aura* triggeredByAura, SpellEntry const* /*procSpell*/, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 /*cooldown*/)
{
    ...
    if (jumps > 0 && GetTypeId() == TYPEID_PLAYER && caster_guid.IsPlayer())
    {
        ...
        {
            uint32 rangeIndex = spellProto->rangeIndex;
            radius = GetSpellMaxRange(sSpellRangeStore.LookupEntry(rangeIndex));
        }
...
SpellAuraProcResult Unit::HandleReflectSpellsSchoolAuraProc(Unit* /*pVictim*/, uint32 /*damage*/, Aura* triggeredByAura, SpellEntry const* procSpell, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 /*cooldown*/)
{
    ...
    uint32 schoolMask = procSpell->SchoolMask;

    return !(procSpell == NULL || (triggeredByAura->GetModifier()->m_miscvalue & schoolMask) == 0) ? SPELL_AURA_PROC_OK : SPELL_AURA_PROC_FAILED;
}
...
SpellAuraProcResult Unit::HandleModPowerCostSchoolAuraProc(Unit* /*pVictim*/, uint32 /*damage*/, Aura* triggeredByAura, SpellEntry const* procSpell, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 /*cooldown*/)
{
    ...
    return !(procSpell == NULL ||
            ...
            (triggeredByAura->GetModifier()->m_miscvalue & schoolMask) == 0) ? SPELL_AURA_PROC_OK : SPELL_AURA_PROC_FAILED;  // School check
}
...
SpellAuraProcResult Unit::HandleModDamagePercentDoneAuraProc(Unit* /*pVictim*/, uint32 /*damage*/, Aura* triggeredByAura, SpellEntry const* procSpell, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 cooldown)
{
    ...
    uint32 procSpellIcon = procSpell->SpellIconID;
    ...
    // Arcane Blast
    else if (spellInfo->Id == 36032 && procSpell->GetSpellFamilyName() == SPELLFAMILY_MAGE && procSpellIcon == 2294)
