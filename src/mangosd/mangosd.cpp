 ...
 * - Process-level setup (OpenSSL, PID file, console writer)
 * - Handing off to Master, which owns the databases, the world loop and
 *   every background service (remote administration, SOAP, the freeze
 *   watchdog, the console reader)
 *
...
int main(int argc, char** argv)
{
    ...
    // the fallible init above (OpenSSL / PID file) so the early-return error
    // paths never leave a writer thread running into stdio teardown; still
    // placed before Master::Run() -- SetInitialWorldSettings()'s LivingWorld
    // spawn burst happens inside it -- so the hot console path is covered.
    ...
    // online flags, sent queued mass mail and halted the database delay
    // threads. What is left here is process-level teardown only.
    ...
    // Stop and join the off-thread console writer before the final shutdown
    // lines: later lines ("Bye!") then take the synchronous fallback. Every
    // thread that can emit to the console -- the map-update workers, the
    // network workers and all background services -- has already been joined
    // by Master::Run() before this point, so nothing can race the writer's
    // deletion.
