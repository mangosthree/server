...
// The auction-house update formerly ran on an ACE_Task; ACE was removed from
// the core, so it now runs on a detached std::thread (fire-and-forget;
// ForceUpdate guards against re-entry via the `updating` flag). AhBot is a
// singleton, so capturing `this` in the detached thread is safe.
...
int AhBot::AddAuction(int auction, Category* category, ItemPrototype const* proto)
{
    ...
    string name;
    if (!sObjectMgr.GetPlayerNameByGUID(ObjectGuid((uint64)owner), name))
    {
        return 0;
    }
...
uint32 AhBot::GetRandomBidder(uint32 auctionHouse)
{
    ...
    for (vector<uint32>::iterator i = guids.begin(); i != guids.end(); ++i)
    {
        uint32 guid = *i;
        string name;
        if (!sObjectMgr.GetPlayerNameByGUID(ObjectGuid((uint64)guid), name))
        {
            continue;
        }
...
void AhBot::CheckSendMail(uint32 bidder, uint32 price, AuctionEntry *entry)
{
    ...
    string name;
    if (!sObjectMgr.GetPlayerNameByGUID(ObjectGuid((uint64)bidder), name))
    {
        return;
    }
