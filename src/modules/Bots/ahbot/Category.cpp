...
bool TradeSkill::IsCraftedBySpell(ItemPrototype const* proto, uint32 spellId)
{
    ...
    SpellReagentsEntry const* reagents = entry->GetSpellReagents();
    for (uint32 x = 0; reagents && x < MAX_SPELL_REAGENTS; ++x)
    {
        if (reagents->Reagent[x] <= 0)
        {
            continue;
        }

        if (proto->ItemId == reagents->Reagent[x])
        {
...
bool TradeSkill::IsCraftedBy(ItemPrototype const* proto, uint32 spellId)
{
    ...
    {
        SpellEffectEntry const* effectEntry = entry->GetSpellEffect(SpellEffectIndex(effect));
        if (!effectEntry)
        {
            continue;
        }
        uint32 craftId = effectEntry->EffectTriggerSpell;
        ...
        SpellReagentsEntry const* craftReagents = craft->GetSpellReagents();
        for (uint32 i = 0; craftReagents && i < MAX_SPELL_REAGENTS; ++i)
        {
            uint32 itemId = craftReagents->Reagent[i];
