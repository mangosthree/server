    ...
    m_eatingUntil(0), m_drinkingUntil(0)
{
    ...
    masterOutgoingPacketHandlers.AddHandler(SMSG_FORCE_FLIGHT_SPEED_CHANGE, "check mount state");
...
bool PlayerbotAI::CastSpell(uint32 spellId, Unit* target)
{
    ...
    if (pSpellInfo->GetSpellEffectIdByIndex(SpellEffectIndex(0)) == SPELL_EFFECT_OPEN_LOCK ||
        pSpellInfo->GetSpellEffectIdByIndex(SpellEffectIndex(0)) == SPELL_EFFECT_SKINNING)
    {
...
bool PlayerbotAI::IsInterruptableSpellCasting(Unit* target, string spell)
{
    ...
    for (int32 i = EFFECT_INDEX_0; i <= EFFECT_INDEX_2; i++)
    {
        ...
        if ((spellInfo->GetSpellEffectIdByIndex(SpellEffectIndex(i)) == SPELL_EFFECT_INTERRUPT_CAST) &&
