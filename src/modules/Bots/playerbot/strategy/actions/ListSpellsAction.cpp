...
bool ListSpellsAction::Execute(Event event)
{
    ...
    for (PlayerSpellMap::iterator itr = bot->GetSpellMap().begin(); itr != bot->GetSpellMap().end(); ++itr)
    {
        ...
        for (uint32 x = 0; reagents && x < MAX_SPELL_REAGENTS; ++x)
        {
            ...
            uint32 itemid = reagents->Reagent[x];
            uint32 reagentsRequired = reagents->ReagentCount[x];
            if (itemid)
            {
        ...
        if (skillLine)
        {
            for (int i = 0; i < 3; ++i)
            {
                if (pSpellInfo->GetSpellEffectIdByIndex(SpellEffectIndex(i)) == SPELL_EFFECT_CREATE_ITEM)
                {
                    ItemPrototype const* proto = sObjectMgr.GetItemPrototype(pSpellInfo->GetSpellEffect(SpellEffectIndex(i))->EffectItemType);
                    if (proto)
                    {
                        if (craftCount)
                        {
