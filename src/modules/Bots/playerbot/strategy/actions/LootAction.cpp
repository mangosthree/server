...
bool OpenLootAction::CanOpenLock(LootObject& lootObject, const SpellEntry* pSpellInfo, GameObject* go)
{
    ...
    {
        if (pSpellInfo->GetSpellEffectIdByIndex(SpellEffectIndex(effIndex)) != SPELL_EFFECT_OPEN_LOCK && pSpellInfo->GetSpellEffectIdByIndex(SpellEffectIndex(effIndex)) != SPELL_EFFECT_SKINNING)
        {
            return false;
        }
        ...
        for(int j = 0; j < 8; ++j)
        {
            ...
            */
            case LOCK_KEY_SKILL:
                {
                    if (uint32(pSpellInfo->GetEffectMiscValue(SpellEffectIndex(effIndex))) != lockInfo->Index[j])
                    {
                        continue;
                    }
