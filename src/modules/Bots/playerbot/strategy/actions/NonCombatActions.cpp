...
bool DrinkAction::Execute(Event event)
{
    ...
    {
        // Cata 4.3.4 has no conjured drink item -- Conjure Refreshment only
        // produces conjured food, whose use-spell restores both health and
        // mana (verified via Spell.dbc/SpellEffect.dbc, see #381 commit
        // message). Fall back to it before the normal drink list.
        list<Item*> conjuredFoods = AI_VALUE2(list<Item*>, "inventory items", "conjured food");
        if (!conjuredFoods.empty())
        {
            result = UseItemAuto(*conjuredFoods.begin());
        }
        else
        {
