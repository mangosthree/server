...
bool TeleportAction::Execute(Event event)
{
    ...
    for (list<ObjectGuid>::iterator i = gos.begin(); i != gos.end(); i++)
    {
        ...
        if (pSpellInfo->GetSpellEffectIdByIndex(SpellEffectIndex(0)) != SPELL_EFFECT_TELEPORT_UNITS && pSpellInfo->GetSpellEffectIdByIndex(SpellEffectIndex(1)) != SPELL_EFFECT_TELEPORT_UNITS && pSpellInfo->GetSpellEffectIdByIndex(SpellEffectIndex(2)) != SPELL_EFFECT_TELEPORT_UNITS)
        {
            continue;
        }
