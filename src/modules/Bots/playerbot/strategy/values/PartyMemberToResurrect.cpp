...
public:
    virtual bool Check(SpellEntry const* spell)
    {
        for (int i=0; i<3; i++)
        {
            if (spell->GetSpellEffectIdByIndex(SpellEffectIndex(i)) == SPELL_EFFECT_RESURRECT ||
                spell->GetSpellEffectIdByIndex(SpellEffectIndex(i)) == SPELL_EFFECT_RESURRECT_NEW ||
                spell->GetSpellEffectIdByIndex(SpellEffectIndex(i)) == SPELL_EFFECT_SELF_RESURRECT)
                return true;
        }
        return false;
    }
