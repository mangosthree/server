...
struct boss_midnight : public CreatureScript
{
    ...
    struct boss_midnightAI : public ScriptedAI
    {
        ...
        {
            if (eventType == AI_EVENT_CUSTOM_A && pSender == m_creature && pInvoker->GetTypeId() == TYPEID_UNIT)
            {
                if (pInvoker)   // Wrapper to prepare phase 3
                {
                    m_uiPhase = 2;
...
struct boss_attumen : public CreatureScript
{
...
struct spell_mount_attumen : public SpellScript
{
    spell_mount_attumen() : SpellScript("spell_mount_attumen") {}
    ...
    bool EffectDummy(Unit* pCaster, uint32 uiSpellId, SpellEffectIndex uiEffIndex, Object* pTarget, ObjectGuid /*originalCasterGuid*/) override
    {
        ...
        if (uiSpellId == SPELL_MOUNT && uiEffIndex == EFFECT_INDEX_0)
        {
            ...
            {
                // Prepare for mount
                if (CreatureAI* pMidnightAI = ((Creature*)pCaster)->AI())
                {
                    pMidnightAI->ReceiveAIEvent(AI_EVENT_CUSTOM_A, pCaster->ToCreature(), pCreatureTarget, 0);
