...
struct npc_barnes : public CreatureScript
{
        ...
        switch (uiAction)
        {
            ...
            case GOSSIP_ACTION_INFO_DEF+3:
                ...
                if (ScriptedInstance* pInstance = (ScriptedInstance*)pCreature->GetInstanceData())
                {
                    ...
                    outstring_log("SD3: %s manually set Opera event to EVENT_OZ", pPlayer->GetGuidStr().c_str());
                }
                break;
            case GOSSIP_ACTION_INFO_DEF+4:
                ...
                if (ScriptedInstance* pInstance = (ScriptedInstance*)pCreature->GetInstanceData())
                {
                    ...
                    outstring_log("SD3: %s manually set Opera event to EVENT_HOOD", pPlayer->GetGuidStr().c_str());
                }
                break;
            case GOSSIP_ACTION_INFO_DEF+5:
                ...
                if (ScriptedInstance* pInstance = (ScriptedInstance*)pCreature->GetInstanceData())
                {
                    ...
                    outstring_log("SD3: %s manually set Opera event to EVENT_RAJ", pPlayer->GetGuidStr().c_str());
                }
                break;
        }
...
struct npc_berthold : public CreatureScript
{
...
struct npc_image_of_medivh : public CreatureScript
{
...
struct npc_image_arcanagos : public CreatureScript
{
...
struct event_spell_medivh_journal : public MapEventScript
{
    event_spell_medivh_journal() : MapEventScript("event_spell_medivh_journal") {}
    ...
    bool OnReceived(uint32 /*uiEventId*/, Object* pSource, Object* /*pTarget*/, bool bIsStart) override
    {
        if (bIsStart && pSource->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            if (Creature* pMedivh = ((Player*)pSource)->SummonCreature(NPC_IMAGE_OF_MEDIVH, afMedivhSpawnLoc[0], afMedivhSpawnLoc[1], afMedivhSpawnLoc[2], afMedivhSpawnLoc[3], TEMPSPAWN_DEAD_DESPAWN, 0))
            {
                ...
                if (CreatureAI* pMedivhAI = pMedivh->AI())
                {
                    pMedivhAI->ReceiveAIEvent(AI_EVENT_CUSTOM_A, pMedivh, pSource->ToPlayer(), 0);
...
void AddSC_karazhan()
{
    ...
    s = new npc_berthold();
    s->RegisterSelf();
    s = new npc_image_of_medivh();
    s->RegisterSelf();
    s = new npc_image_arcanagos();
    s->RegisterSelf();
    s = new event_spell_medivh_journal();
    s->RegisterSelf();
