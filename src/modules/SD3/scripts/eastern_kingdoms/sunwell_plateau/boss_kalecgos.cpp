...
struct boss_kalecgos : public CreatureScript
{
    ...
    struct boss_kalecgosAI : public ScriptedAI
    {
        ...
        void JustReachedHome() override
        {
            if (m_pInstance)
            {
                m_pInstance->SetData(TYPE_KALECGOS_EJECT_SPECTRAL, 0);
...
struct boss_sathrovarr : public CreatureScript
{
    ...
    struct boss_sathrovarrAI : public ScriptedAI
    {
        ...
        void DamageTaken(Unit* /*pDoneBy*/, uint32& uiDamage) override
        {
            if (uiDamage > m_creature->GetHealth())
            {
                ...
                {
                    if (CreatureAI* pKalecgosAI = pKalecgos->AI())
                    {
                        pKalecgosAI->ReceiveAIEvent(AI_EVENT_CUSTOM_B, pKalecgos, pKalecgos, 0);
                    }
                }
...
struct boss_kalecgos_humanoid : public CreatureScript
{
    boss_kalecgos_humanoid() : CreatureScript("boss_kalecgos_humanoid") {}
...
struct spell_spectral_realm_notify : public SpellScript
{
    spell_spectral_realm_notify() : SpellScript("spell_spectral_realm_notify") {}
...
void AddSC_boss_kalecgos()
{
    ...
    s = new spell_spectral_realm_notify();
    s->RegisterSelf();
    s = new boss_kalecgos();
    s->RegisterSelf();
    s = new boss_sathrovarr();
    s->RegisterSelf();
    s = new boss_kalecgos_humanoid();
    s->RegisterSelf();
