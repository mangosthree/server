...
struct npc_medivh_black_morass : public CreatureScript
{
...
struct spell_medivh_black_morass : public SpellScript
{
    spell_medivh_black_morass() : SpellScript("spell_medivh_black_morass") {}
...
struct npc_time_rift : public CreatureScript
{
    ...
    struct npc_time_riftAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            if (m_pInstance)
            {
                m_uiRiftNumber = m_pInstance->GetData(TYPE_RIFT_ID);
...
struct spell_npc_time_rift_channel : public SpellScript
{
    spell_npc_time_rift_channel() : SpellScript("spell_npc_time_rift_channel") {}
    ...
    bool EffectDummy(Unit* /*pCaster*/, uint32 uiSpellId, SpellEffectIndex uiEffIndex, Object* pTarget, ObjectGuid /*originalCasterGuid*/) override
    {
        ...
        {
            if (CreatureAI* pTimeRiftAI = pTarget->ToCreature()->AI())
            {
                pTimeRiftAI->ReceiveAIEvent(AI_EVENT_CUSTOM_A, pTarget->ToCreature(), pTarget->ToCreature(), 0);
            }
...
void AddSC_dark_portal()
{
    ...
    s = new npc_medivh_black_morass();
    s->RegisterSelf();
    s = new spell_medivh_black_morass();
    s->RegisterSelf();
    s = new npc_time_rift();
    s->RegisterSelf();
    s = new spell_npc_time_rift_channel();
    s->RegisterSelf();
