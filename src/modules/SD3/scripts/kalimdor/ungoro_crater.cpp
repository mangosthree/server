...
enum
{
    ...
    GOSSIP_ITEM_SIMONE_THE_INCONSPICUOUS = -3509004,
...
struct npc_precious_the_devourerAI : public ScriptedAI
{
    ...
    void SetSimoneGuidForAI(ObjectGuid pSimoneGuid)
    {
        m_simoneGuid = pSimoneGuid;
    }
        ...
        if (Creature* pPrecious = m_creature->GetMap()->GetCreature(m_preciousGuid))
        {
            if (!pPrecious->IsAlive())
            {
                ...
                if (pPreciousNew)
                {
                    ...
                    npc_precious_the_devourerAI* preciousAI = dynamic_cast<npc_precious_the_devourerAI*>(pPreciousNew->AI());

                    if (preciousAI)
                    {
                        preciousAI->SetSimoneGuidForAI(m_creature->GetObjectGuid());
                    }
                }
            }
        }
...
struct npc_simone_the_seductress : public CreatureScript
{
    ...
    CreatureAI* GetAI(Creature* pCreature) override
    {
        return new npc_simone_the_seductressAI(pCreature);
    }
};
...
struct npc_simone_the_inconspicuousAI : public ScriptedAI
{
    ...
    void Transform()
    {
        if (m_creature->GetMap()->GetPlayer(m_playerGuid))
        {
            ...
            if (pDemon)
            {
                if (npc_simone_the_seductressAI* pSimone = dynamic_cast<npc_simone_the_seductressAI*> (pDemon->AI()))
                {
...
void AddSC_ungoro_crater()
{
    ...
    s = new npc_precious_the_devourer();
    s->RegisterSelf();

    s = new npc_simone_the_seductress();
    s->RegisterSelf();

    s = new npc_simone_the_inconspicuous();
    s->RegisterSelf();
}
