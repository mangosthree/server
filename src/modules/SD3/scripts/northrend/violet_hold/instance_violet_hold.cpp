...
struct is_violet_hold : public InstanceScript
{
    ...
    public:
        ...
        {
            debug_log("SD3: instance_violet_hold: SetData got type %u, data %u.", uiType, uiData);
    ...
    private:
        ...
        void SetPortalId()
        {
            if (IsCurrentPortalForTrash())
            {
                ...
                debug_log("SD3: instance_violet_hold: SetPortalId %u, old was id %u.", uiTemp, m_uiPortalId);

                m_uiPortalId = uiTemp;
            }
            else if (m_uiWorldStatePortalCount == 18)
            {
                debug_log("SD3: instance_violet_hold: SetPortalId %u (Cyanigosa), old was id %u.", 0, m_uiPortalId);
                m_uiPortalId = 0;
            }
            else
            {
                debug_log("SD3: instance_violet_hold: SetPortalId %u (is boss), old was id %u.", m_uiMaxCountPortalLoc, m_uiPortalId);
