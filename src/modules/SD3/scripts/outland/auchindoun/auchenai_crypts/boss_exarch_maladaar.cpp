...
struct mob_stolen_soul : public CreatureScript
{
    ...
    struct mob_stolen_soulAI : public ScriptedAI
    {
        ...
        {
            if (eventType == AI_EVENT_CUSTOM_A && pInvoker->GetTypeId() == TYPEID_PLAYER && pSender->GetEntry() == 18373)   // 18373 Exarch Maladaar
            {
                m_uiStolenClass = pInvoker->getClass();
                m_targetGuid = pInvoker->GetObjectGuid();
                m_creature->SetDisplayId(pInvoker->GetDisplayId());
            }
        }
...
struct boss_exarch_maladaar : public CreatureScript
{
...
void AddSC_boss_exarch_maladaar()
{
    ...
    s = new boss_exarch_maladaar();
    s->RegisterSelf();
    s = new mob_stolen_soul();
    s->RegisterSelf();
