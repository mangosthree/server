...
struct npc_akama : public CreatureScript
{
    npc_akama() : CreatureScript("npc_akama_shade") {}
    ...
    struct npc_akamaAI : public ScriptedAI, private DialogueHelper
    {
        ...
        void DoSummonAshtongue(uint32 uiSpellId = 0)
        {
            ...
            uint64 guid = m_pInstance->GetData64(TYPE_CHANNELERS);
            Creature* pGenerator = m_creature->GetMap()->GetCreature(ObjectGuid(guid));
            if (!pGenerator)
            {
                return;
            }
        ...
        void DoSummonBrokenAshtongue()
        {
            ...
            m_pInstance->SetData64(TYPE_CHANNELERS, m_creature->GetObjectGuid().GetRawValue());
        }
...
struct boss_shade_of_akama : public CreatureScript
{
...
struct mob_ashtongue_channeler : public CreatureScript
{
...
struct mob_ashtongue_sorcerer : public CreatureScript
{
...
void AddSC_boss_shade_of_akama()
{
    ...
    s = new npc_akama();
    s->RegisterSelf();
    s = new boss_shade_of_akama();
    s->RegisterSelf();
    s = new mob_ashtongue_channeler();
    s->RegisterSelf();
    s = new mob_ashtongue_sorcerer();
    s->RegisterSelf();
