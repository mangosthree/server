...
struct boss_lady_vashj : public CreatureScript
{
    ...
    struct boss_lady_vashjAI : public ScriptedAI
    {
        ...
        void MovementInform(uint32 uiType, uint32 uiPointId) override
        {
            ...
            if (uiPointId == POINT_MOVE_CENTER)
            {
                ...
                if (m_pInstance)
                {
                    m_pInstance->SetData(TYPE_DO_VASHJ_GENERATORS, m_creature->GetObjectGuid().GetRawValue());
                }
        ...
        void ReceiveAIEvent(AIEventType eventType, Creature *pSender, Unit* /*pReceiver*/, uint32 /*data*/) override
        {
...
struct mob_enchanted_elemental : public CreatureScript
{
...
struct go_shield_generator : public GameObjectScript
{
    go_shield_generator() : GameObjectScript("go_shield_generator") {}
    ...
    bool OnUse(Player* /*pPlayer*/, GameObject* pGo) override
    {
        ...
        if (ScriptedInstance* pInstance = (ScriptedInstance*)pGo->GetInstanceData())
        {
            ...
            {
                if (CreatureAI* pLadyAI = pVashj->AI())
                {
                    pLadyAI->ReceiveAIEvent(AI_EVENT_CUSTOM_A, pVashj, pVashj, 0);
                }
            }
...
void AddSC_boss_lady_vashj()
{
    ...
    s = new boss_lady_vashj();
    s->RegisterSelf();
    s = new mob_enchanted_elemental();
    s->RegisterSelf();
    s = new go_shield_generator();
    s->RegisterSelf();
