...
struct mob_mature_netherwing_drake : public CreatureScript
{
...
struct mob_enslaved_netherwing_drake : public CreatureScript
{
...
struct npc_dragonmaw_peon : public CreatureScript
{
...
struct spell_serving_mutton : public SpellScript
{
    spell_serving_mutton() : SpellScript("spell_serving_mutton") {}
    ...
    bool EffectDummy(Unit* pCaster, uint32 uiSpellId, SpellEffectIndex uiEffIndex, Object* pTarget, ObjectGuid /*originalCasterGuid*/) override
    {
        ...
        CreatureAI* pPeonAI = pCreatureTarget->AI();

        if (pPeonAI)
        {
            pPeonAI->ReceiveAIEvent(AI_EVENT_CUSTOM_A, pCreatureTarget, pCaster, 0);
            return true;
        }
...
struct npc_wilda : public CreatureScript
{
    npc_wilda() : CreatureScript("npc_wilda") {}
...
struct mob_torloth : public CreatureScript
{
    ...
    struct mob_torlothAI : public ScriptedAI
    {
        ...
        void ReceiveAIEvent(AIEventType eventType, Creature* pSender, Unit* pInvoker, uint32 /*data*/) override
        {
            ...
            {
                m_lordIllidanGuid = pSender->GetObjectGuid();
                m_playerGuid = pInvoker->GetObjectGuid();
            }
        }
...
struct npc_lord_illidan_stormrage : public CreatureScript
{
    ...
    struct npc_lord_illidan_stormrageAI : public Scripted_NoMovementAI
    {
        ...
        void JustSummoned(Creature* pSummoned) override
        {
            ...
            Player* p = m_creature->GetMap()->GetPlayer(m_playerGuid);
            if (!p)
            {
                return;
            }
...
struct npc_totem_of_spirits : public CreatureScript
{
...
struct spell_totem_of_spirits : public SpellScript
{
    spell_totem_of_spirits() : SpellScript("spell_totem_of_spirits") {}
...
struct aura_elemental_sieve : public AuraScript
{
    ...
    {
        if (pAura->GetId() != SPELL_ELEMENTAL_SIEVE || pAura->GetEffIndex() != EFFECT_INDEX_0)
        {
            return false;
        }
...
{
    event_spell_soul_captured_credit() : MapEventScript("event_spell_soul_captured_credit") {}
...
struct npc_spawned_oronok_tornheart : public CreatureScript
{
    npc_spawned_oronok_tornheart() : CreatureScript("npc_spawned_oronok_tornheart") {}
...
struct npc_domesticated_felboar : public CreatureScript
{
    npc_domesticated_felboar() : CreatureScript("npc_domesticated_felboar") {}
...
struct spell_tuber_whistle : public SpellScript
{
    spell_tuber_whistle() : SpellScript("spell_tuber_whistle") {}
...
struct npc_veneratus_spawn_node : public CreatureScript
{
    npc_veneratus_spawn_node() : CreatureScript("npc_veneratus_spawn_node") {}
...
void AddSC_shadowmoon_valley()
{
    ...
    s = new mob_mature_netherwing_drake();
    s->RegisterSelf();
    s = new mob_enslaved_netherwing_drake();
    s->RegisterSelf();
    s = new npc_dragonmaw_peon();
    s->RegisterSelf();
    s = new npc_wilda();
    s->RegisterSelf();
    s = new npc_lord_illidan_stormrage();
    s->RegisterSelf();
    s = new mob_torloth();
    s->RegisterSelf();
    s = new npc_totem_of_spirits();
    s->RegisterSelf();
    s = new event_spell_soul_captured_credit();
    s->RegisterSelf();
    s = new go_crystal_prison();
    s->RegisterSelf();
    s = new npc_spawned_oronok_tornheart();
    s->RegisterSelf();
    s = new npc_domesticated_felboar();
    s->RegisterSelf();
    s = new npc_veneratus_spawn_node();
    s->RegisterSelf();
    s = new spell_tuber_whistle();
    s->RegisterSelf();
