...
class ByteBufferException : public std::exception
{
    public:
        ...
        char const* what() const throw() override { return _message.c_str(); }

    protected:
        std::string& message() throw() { return _message; }
...
class ByteBuffer
{
    public:
        ...
        ByteBuffer(ByteBuffer&& buf) : _rpos(buf._rpos), _wpos(buf._wpos), _bitpos(buf._bitpos),
        ...
        ByteBuffer(ByteBuffer const& right) : _rpos(right._rpos), _wpos(right._wpos), _bitpos(right._bitpos),
            _curbitval(right._curbitval), _storage(std::move(right._storage)) { }
        ...
        */
        ...
        template <typename T>
        void PutBits(size_t rpos, T value, uint32 bitCount)
        {
            if (!bitCount)
            {
        ...
        void readPacketGUID(uint64& guid)
        {
            if (rpos() + 1 > size())
            {
