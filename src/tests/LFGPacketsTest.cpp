...
TEST(LFGPackets_PlayerInfo_call_to_arms_tier)
{
    // What a level 85 random heroic looks like on the wire while the queue
    // is calling for tanks: severity tier 0 carries the satchel, the other
    // two severities stay zero-masked and are skipped entirely.
    LFGPackets::LFGRandomDungeonEntry entry;
    entry.slot = 301 | (6u << 24);                    // random heroic slot
    entry.shortageEligible = true;
    entry.shortageTiers[0].roleMask = 2;              // tank
    entry.shortageTiers[0].items = { { 69903, 98856, 1, false } };
