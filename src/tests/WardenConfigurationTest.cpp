...
warden::WardenRawConfiguration ValidCustomConfiguration()
{
    ...
    raw.requireCurrentX86Patch = false;
...
TEST(WardenConfiguration_defaults_match_safe_provisional_policy)
{
    ...
    CHECK(result.value.requireCurrentX86Patch);
...
TEST(WardenConfiguration_unloaded_incident_history_never_selects_aggressive)
{
    ...
    history.aggressiveUntilServer = 2000;
    history.incidentHistoryLoaded = false;

    CHECK(!warden::IsWardenAdmissionAggressive(history, configuration, 1000));
}

TEST(WardenConfiguration_admission_aggression_requires_current_threshold)
{
    warden::WardenConfiguration configuration;
    configuration.aggressiveThreshold = 5;
    warden::WardenAdmissionHistory history;
    history.recentIncidentCount = 4;
    history.aggressiveUntilServer = 2000;
    history.incidentHistoryLoaded = true;

    CHECK(!warden::IsWardenAdmissionAggressive(history, configuration, 1000));
    history.recentIncidentCount = 5;
    CHECK(warden::IsWardenAdmissionAggressive(history, configuration, 1000));
}

TEST(WardenConfiguration_absolute_aggression_deadline_expires_while_queued)
{
    warden::WardenConfiguration configuration;
    warden::WardenAdmissionHistory history;
    history.recentIncidentCount = configuration.aggressiveThreshold;
    history.aggressiveUntilServer = 1500;
    history.incidentHistoryLoaded = true;

    CHECK(warden::IsWardenAdmissionAggressive(history, configuration, 1499));
    CHECK(!warden::IsWardenAdmissionAggressive(history, configuration, 1500));
    CHECK(!warden::IsWardenAdmissionAggressive(history, configuration, 2000));
}
