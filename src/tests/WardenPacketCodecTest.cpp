...
warden::CheckPlan FourFamilyX86Plan()
{
    ...
    plan.profileKey = {15595, warden::WardenArchitecture::X86,
        {{'e', 'n', 'U', 'S'}}, warden::ClientVariant::Grunt};
    ...
    timing.payload = warden::TimingCheckProfile{2001};
    ...
    lua.payload = warden::LuaCheckProfile{2003, "OKAY", "Okay"};
    plan.checks.push_back(lua);

    warden::MpqCheckProfile mpqProfile;
    mpqProfile.checkId = 2002;
    mpqProfile.path = "DBFilesClient\\Item.db2";
    mpqProfile.expectedSha1 = {{
        0x47, 0x06, 0xFF, 0x83, 0xD9, 0xB6, 0x11, 0x64,
        0x4A, 0x87, 0xDE, 0x79, 0xC2, 0x44, 0xB4, 0x14,
        0x61, 0x2E, 0xF4, 0xF2}};
    warden::WardenCheckDefinition mpq;
    mpq.sortOrder = 35;
    mpq.evidenceClass = warden::WardenEvidenceClass::Corroboration;
    ...
    memory.payload = warden::MemCheckProfile{2004,
        {'W', 'o', 'w', '.', 'e', 'x', 'e'}, 0x0043C257, 12,
        {0x53, 0x8B, 0x5D, 0x08, 0x56, 0x8B, 0xC3, 0x57,
            0x8D, 0x50, 0x01, 0x8A}};
    plan.checks.push_back(memory);
    return plan;
}

warden::CheckPlan FourFamilyX64Plan()
{
    warden::CheckPlan plan;
    plan.requestId = 8;
    plan.purpose = warden::CheckPlanPurpose::Initial;
...
warden::ModuleProfile X64FullProfile()
{
    ...
    profile.checkCodes = {warden::Cata15595X64TimingCode,
        warden::Cata15595X64LuaCode, warden::Cata15595X64MpqCode,
        warden::Cata15595X64MemoryCode};
    return profile;
}

warden::ModuleProfile X86FullProfile()
{
    warden::ModuleProfile profile;
    profile.key = {15595, warden::WardenArchitecture::X86};
...
{
    return {
        0x02, 0x2D, 0x00, 0xB9, 0x14, 0x48, 0xF1,
        0x01, 0x78, 0x56, 0x34, 0x12,
        0x00, 0x04, 0x4F, 0x6B, 0x61, 0x79,
        0x00, 0x47, 0x06, 0xFF, 0x83, 0xD9, 0xB6, 0x11,
        0x64, 0x4A, 0x87, 0xDE, 0x79, 0xC2, 0x44, 0xB4,
        0x14, 0x61, 0x2E, 0xF4, 0xF2,
        0x00, 0x53, 0x8B, 0x5D, 0x08, 0x56, 0x8B, 0xC3,
        0x57, 0x8D, 0x50, 0x01, 0x8A};
}

warden::Bytes ValidFourFamilyX64Result()
{
    return {
        0x02, 0x31, 0x00, 0x74, 0xA9, 0x5F, 0x45,
        0x01, 0x78, 0x56, 0x34, 0x12,
        0x00, 0x04, 0x4F, 0x6B, 0x61, 0x79,
        0x00, 0x47, 0x06, 0xFF, 0x83, 0xD9, 0xB6, 0x11,
        0x64, 0x4A, 0x87, 0xDE, 0x79, 0xC2, 0x44, 0xB4,
...
TEST(WardenPacketCodec_encodes_deferred_filesystem_only_for_current_x86_grunt)
{
    warden::Bytes plaintext = {0xA5};
    CHECK_EQ(int(warden::EncodeDeferredFilesystemInitialization(
                 X86FullProfile(), warden::ClientVariant::Grunt, plaintext)),
        int(warden::EncodeStatus::Ok));
    CHECK_HEX(plaintext.data(), plaintext.size(),
             ...
             warden::ClientVariant::LegacyGrunt})
    {
        plaintext = {0xA5};
        CHECK_EQ(int(warden::EncodeDeferredFilesystemInitialization(
                     X86FullProfile(), variant, plaintext)),
            int(warden::EncodeStatus::InvalidProfile));
        CHECK(plaintext.empty());
    }

    warden::ModuleProfile invalid = X64FullProfile();
    plaintext = {0xA5};
    CHECK_EQ(int(warden::EncodeDeferredFilesystemInitialization(
                 invalid, warden::ClientVariant::Grunt, plaintext)),
        int(warden::EncodeStatus::InvalidProfile));
    CHECK(plaintext.empty());

    invalid = X86FullProfile();
    invalid.checkCodes.mpq ^= 0x01;
    plaintext = {0xA5};
    CHECK_EQ(int(warden::EncodeDeferredFilesystemInitialization(
                 invalid, warden::ClientVariant::Grunt, plaintext)),
        int(warden::EncodeStatus::InvalidProfile));
...
TEST(WardenPacketCodec_x64_probe_only_initialization_withholds_filesystem_callbacks)
{
    warden::ModuleProfile profile = X64FullProfile();
    profile.operatingMode = warden::ModuleOperatingMode::CompatibilityProbeOnly;
    profile.checkCodes = {warden::CataX64CompatibilityTimingCode, 0, 0, 0};

    warden::Bytes plaintext = {0xA5};
    CHECK_EQ(int(warden::EncodeModuleInitialization(profile, plaintext)),
        int(warden::EncodeStatus::Ok));
    CHECK_HEX(plaintext.data(), plaintext.size(),
        "030c00f717b38304000010815600c06b560001"
        "0308009906767a01010000255b0001");
...
TEST(WardenPacketCodec_rejects_probe_only_initialization_for_x86)
{
    warden::ModuleProfile profile = X86FullProfile();
    profile.operatingMode = warden::ModuleOperatingMode::CompatibilityProbeOnly;
    profile.checkCodes = {warden::Cata15595X86TimingCode, 0, 0, 0};
...
TEST(WardenPacketCodec_x86_encodes_mpq_only_for_current_adapter_grunt)
{
    ...
    CHECK_EQ(budget.stringTableBytes, std::size_t(37));
    CHECK_EQ(budget.requestBody, std::size_t(51));
    CHECK_EQ(budget.maximumResultBody, std::size_t(296));

    std::optional<warden::WardenCheckXorKey> const key = X86CheckXorKey();
    REQUIRE(key.has_value());
    warden::Bytes encoded;
    CHECK(warden::EncodeCheckRequest(X86FullProfile(), *key, plan, encoded) ==
        warden::EncodeStatus::Ok);
    CHECK_HEX(encoded.data(), encoded.size(),
        "02044f4b415916444246696c6573436c69656e745c4974656d2e646232"
        "07576f772e657865005acb010802be030657c2430c53");

    for (warden::ClientVariant const variant :
        {warden::ClientVariant::Stock, warden::ClientVariant::LegacyGrunt})
    {
        plan.profileKey.variant = variant;
        encoded = {0xA5};
        CHECK(warden::EncodeCheckRequest(
            X86FullProfile(), *key, plan, encoded) ==
            warden::EncodeStatus::InvalidPlan);
        CHECK_HEX(encoded.data(), encoded.size(), "a5");
    }

    plan.profileKey.variant = warden::ClientVariant::Unclassified;
    encoded = {0xA5};
    CHECK(warden::EncodeCheckRequest(X86FullProfile(), *key, plan, encoded) ==
        warden::EncodeStatus::InvalidPlan);
    CHECK_HEX(encoded.data(), encoded.size(), "a5");
...
TEST(WardenPacketCodec_x86_encodes_supported_timing_lua_and_memory_plan)
{
    ...
    plan.profileKey.variant = warden::ClientVariant::Stock;
    ...
    CHECK(warden::EncodeCheckRequest(X86FullProfile(), *key, plan, encoded) ==
        warden::EncodeStatus::Ok);
    CHECK_HEX(encoded.data(), encoded.size(),
        "02044f4b415907576f772e65786500"
        "5acb01be020657c2430c53");
}

TEST(WardenPacketCodec_x64_encodes_pinned_four_family_production_plan)
{
    ...
    CHECK(warden::EncodeCheckRequest(X64FullProfile(), *key,
        FourFamilyX64Plan(), encoded) == warden::EncodeStatus::Ok);
    CHECK_HEX(encoded.data(), encoded.size(),
        "02044f4b415916444246696c6573436c69656e745c4974656d2e646232"
        "0a576f772d36342e65786500bf0401c502630306136c561055");
}

TEST(WardenPacketCodec_x64_decodes_pinned_four_family_production_result)
{
    warden::Bytes const plaintext = ValidFourFamilyX64Result();
    REQUIRE(plaintext.size() == std::size_t(56));

    warden::CheckBatchResult decoded;
    CHECK(warden::DecodeCheckResult(warden::ModuleAbi::Cata15595X64,
        warden::ByteView(plaintext), FourFamilyX64Plan(), decoded) ==
...
{
    warden::Bytes plaintext = ValidFourFamilyX64Result();
...
{
    warden::CheckPlan plan = FourFamilyX64Plan();
    plan.checks.erase(plan.checks.begin(), plan.checks.begin() + 3);
...
{
    warden::CheckPlan x64 = FourFamilyX64Plan();
    x64.checks.erase(x64.checks.begin(), x64.checks.begin() + 3);
...
TEST(WardenPacketCodec_x64_rejects_unpublished_check_code_maps)
{
    std::optional<warden::WardenCheckXorKey> const key =
        X64CheckXorKey();
    REQUIRE(key.has_value());
...
TEST(WardenPacketCodec_x86_decodes_exact_four_family_result_transactionally)
{
    ...
    CHECK(memory->actualBytes ==
        warden::Bytes({0x53, 0x8B, 0x5D, 0x08, 0x56, 0x8B,
            0xC3, 0x57, 0x8D, 0x50, 0x01, 0x8A}));
}
...
{
    // Body offsets: timing 0..4, Lua 5..10, MPQ 11..31, memory 32..44.
...
TEST(WardenPacketCodec_mpq_paths_fit_the_exact_client_stack_buffers)
{
    warden::WardenCheckPlanBudget budget;
    for (warden::ModuleAbi const abi : {warden::ModuleAbi::Cata15595X86,
             warden::ModuleAbi::Cata15595X64})
    {
        warden::CheckPlan plan = abi == warden::ModuleAbi::Cata15595X86 ?
            FourFamilyX86Plan() : FourFamilyX64Plan();
        warden::MpqCheckProfile& mpq =
            std::get<warden::MpqCheckProfile>(plan.checks[2].payload);
        mpq.path.assign(std::numeric_limits<uint8>::max(), 'A');
        CHECK(warden::InspectCheckPlan(abi, plan, budget) ==
            warden::CheckPlanValidation::Valid);

        mpq.path.push_back('A');
        CHECK(warden::InspectCheckPlan(abi, plan, budget) ==
            warden::CheckPlanValidation::InvalidDefinition);
    }
}

TEST(WardenPacketCodec_decodes_mpq_unavailable_without_a_digest)
{
    for (warden::ModuleAbi const abi : {warden::ModuleAbi::Cata15595X86,
             warden::ModuleAbi::Cata15595X64})
    {
        warden::Bytes plaintext = abi == warden::ModuleAbi::Cata15595X86 ?
            ValidFourFamilyResult() : ValidFourFamilyX64Result();
        std::size_t const digestOffset = warden::CheckResultEnvelopeSize + 12;
        plaintext[warden::CheckResultEnvelopeSize + 11] =
            uint8(warden::MpqResultStatus::Unavailable);
        plaintext.erase(plaintext.begin() + digestOffset,
            plaintext.begin() + digestOffset + warden::Digest20{}.size());
        REQUIRE(RewriteCheckResultEnvelope(plaintext));
        ...
        warden::CheckBatchResult decoded;
        CHECK(warden::DecodeCheckResult(abi, warden::ByteView(plaintext),
            plan, decoded) == warden::DecodeStatus::Ok);
        REQUIRE(decoded.checks.size() == 4u);
        warden::MpqResult const* mpq =
            std::get_if<warden::MpqResult>(&decoded.checks[2]);
        REQUIRE(mpq != nullptr);
        CHECK(mpq->status == warden::MpqResultStatus::Unavailable);
    }
}
