...
// The offline baker. One pass over a 4.3.4 client's MPQs, producing the caches
...
"log) everything is baked instead, so an unattended run needs no arguments.\n");
                   ...
                   const std::string& locale)
    {
        ...
        // BOTH client-database formats. Cataclysm added WDB2 (.db2) beside WDBC (.dbc)
        // and put Item, ItemCurrencyCost and ItemExtendedCost in it -- glob only *.dbc
        // and the bake looks complete while the server comes up with no items.
        std::vector<std::string> names = mpq.FindFiles("DBFilesClient\\*.dbc");
        for (const std::string& db2 : mpq.FindFiles("DBFilesClient\\*.db2"))
        {
            names.push_back(db2);
        }
...
int main(int argc, char** argv)
{
    ...
    g_console.Start(opt.src, opt.dest, "4.3.4");
    ...
    StormLibArchive mpq;
    const int opened = mpq.OpenPatchedClientData(
        opt.src, ClientArchives434(), ClientLocaleArchives434(),
        ClientUpdates434(), ClientLocaleUpdates434(), opt.locale);
    if (!opened)
    {
    ...
    if (opt.dbc && opt.allLocales)
    {
        for (const std::string& loc : FindLocales(opt.src))
        {
            ...
            if (!other.OpenPatchedClientData(opt.src, ClientArchives434(),
                                       ClientLocaleArchives434(),
                                       ClientUpdates434(),
                                       ClientLocaleUpdates434(), loc))
            {
